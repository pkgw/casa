...
def query_tmcdb_antennas(ant_names, timestamp):
    ...
    casalog.post('Querying positions for configuration: {0}, for these antennas: {1}, '
                 'at time: {2}'.format(config_name, ant_names_str, timestamp), 'INFO')

    wsclient = Client(SRV_WSDL_URL)
    resp = None
    try:
        resp = wsclient.service.getAntennaPositions(configurationName=config_name,
                                                    antennaNames=ant_names_str,
                                                    timestamp=timestamp)
    except AttributeError as exc:
        import traceback
        traceback.print_exc(file=sys.stdout)
        raise RuntimeError('Got a response back from the server but it is wrong or could '
                           'not be parsed as a valid SOAP response. This occurs for example '
                           'when a wrong timestamp is given. Response object: {0}. '
                           'Error description:'.
                           format(resp, exc))
...
def process_tmcdb_response_for_gencal(resp):
    ...
    def get_ant_pad_position(pos):
        ...
        :returns: tuple: position found, position as a list [x,y,z], pad position
        as a list [x, y, z]
        """
        ant_params = []
        pos_found = False
        ...
        if not pos.completion.status == 'true' or not pos.position:
            ...
            ant_position = np.asarray([pos.position.x, pos.position.y, pos.position.z],
                                      dtype=float)
...
def correct_ant_posns_alma(vis_name, print_offsets=False):
    ...
    def get_ant_pad_names_posns(vis_name):
        ...
        of pad/station names from the MS, 3) positions [x, y, z] of the antennas
        as recorded in the MS.
        """
        ...
        try:
            ...
            if 'ASDM_STATION' not in tb_tool.keywordnames():
                raise RuntimeError('The ANTENNA and/or STATION tables from the ASDM '
            ...
            casalog.post('Could not find pad and antenna position information from '
                         'the ASDM_ANTENNA and ASDM_STATION subtables. Defaulting to '
                         'the POSITION column of the ANTENNA subtable. This means that '
                         'the parameter calculates are most likely inaccurate', 'WARN')
            tb_tool.open(vis_name + '/ANTENNA')
            ant_posns = tb_tool.getcol('POSITION')
            tb_tool.close()

        ant_posns_lists = map(list, zip(ant_posns[0], ant_posns[1], ant_posns[2]))
        return (ant_names, pad_names, ant_posns_lists)

    def get_time_range_from_obs(vis_name):
        observation = tb_tool.open(vis_name + '/OBSERVATION')
    ...
    def build_obs_time(time_range):
        """
        Produces an observation time string, picking the middle point in time between
        start and end of observation.
    ...
    def calc_ant_params_from_positions(ant_names, ant_corr_posns, pad_posns, ant_posns_ms):
        """
        Produces the antenna position correction offset parameters required by gencal.
        Uses positions
        ...
        :returns: position correction parameters for the antennas listed in the first
        parameter
        """
        ...
        def calc_param_diff(ant_corr_pos, pad_pos, ant_pos_ms):
            ...
            :returns: position correction parameters for one antenna (Bx, By, Bz)
            """
            import math

            lat = (math.asin(pad_pos[2] /
                             math.sqrt(pad_pos[0]**2 + pad_pos[1]**2 + pad_pos[2]**2)))
            lon = math.atan2(pad_pos[1], pad_pos[0])

            itrf_diff = pad_pos + ant_corr_pos - ant_pos_ms
            param = np.array([0, 0, 0], dtype=float)
            param[0] = (-math.sin(lon) * itrf_diff[0] - math.cos(lon) * math.sin(lat) *
                        itrf_diff[1] + math.cos(lon) * math.cos(lat) * itrf_diff[2])
            param[1] = (math.cos(lon) * itrf_diff[0] - math.sin(lon) * math.sin(lat) *
                        itrf_diff[1] + math.sin(lon) * math.cos(lat) * itrf_diff[2])
            param[2] = math.cos(lat) * itrf_diff[1] + math.sin(lat) * itrf_diff[2]

            return (param[0], param[1], param[2])

        ant_params = []
        casalog.post('Antenna positions found in TMCDB: {0}, \nPositions found in MS: {1}'.
                     format(ant_corr_posns, ant_posns_ms), 'DEBUG')

        for idx, _name in enumerate(ant_names):
            if np.all(ant_corr_posns[idx] == 0) and np.all(pad_posns[idx] == 0):
                param = np.array([0, 0, 0], dtype=float)
            else:
                # pos_add = ant_corr_posns[idx] + pad_posns[idx]
                # param = pos_add - ant_posns_ms[idx]
                param = calc_param_diff(ant_corr_posns[idx], pad_posns[idx], ant_posns_ms[idx])
    ...
    ant_names_str = ''
    ant_positions = []
    # Get corrected positions by querying the TMC database via the "TMCDBAntennaPad"
    # web service
    try:
        import urllib2
        response = query_tmcdb_antennas(ant_names, obs_time)
        (ant_names_db, ant_corr_posns, pad_posns) = process_tmcdb_response_for_gencal(response)
        if (ant_names_db != ant_names).any():
            raise RuntimeError('The antenna names found in the MS (which were used '
                               'to query the TMCDB) do not match the names returned '
                               'by the database.\nFound in MS: {0}.\nFound in TMCDB: '
                               '{1}'.format(ant_names_db, ant_names))
        ret_code = 0
        ...
        casalog.post('Issue found while querying ALMA TMCDB AntennaPad service. '
                     'Details: {}'.format(exc), 'ERROR')
        ret_code = 1

    ant_names_str, ant_params = calc_ant_params_from_positions(ant_names, ant_corr_posns,
                                                               pad_posns, ant_posns_ms)

    casalog.post('Parameter values produced for gencal: {0}'.format(ant_params), 'DEBUG')
