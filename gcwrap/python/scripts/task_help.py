...
def taskhelp(scrap=None):
    ...
    if scrap:
        scrap = str(scrap)
        foundtasks = [ft for ft in list(tasksum.keys()) if ft.find(scrap) > -1 or tasksum[ft].find(scrap) > -1]
        if not foundtasks:
            print("No tasks were found with '%s' in their name or description." % scrap)
        return
    else:
        print('Available tasks: \n')
        foundtasks = list(tasksum.keys())
...
def tasklist():
    ...
    if 'mytasks' in globals() :
        print('')
        print('User defined tasks')
        print('------------------')
        for key in list(mytasks.keys()) :
            print(key)
