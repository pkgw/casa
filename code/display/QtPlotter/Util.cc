...
namespace casa {
	...
	int Util::getTabularFrequencyAxisIndex(std::shared_ptr<const ImageInterface<Float> > img){
		int tabIndex = -1;
	...
	std::pair<Vector<Float>,Vector<Float> > Util::getProfile(std::shared_ptr<const casacore::ImageInterface<Float> >& imagePtr,
