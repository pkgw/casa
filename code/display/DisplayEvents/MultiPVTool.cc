...
// template bool casa::memory::operator==(std::shared_ptr<casa::viewer::PVLine> const&, casa::viewer::PVLine*);
...
// 		Double minor = beam.getMinor("rad");
	...
	std::shared_ptr<viewer::PVLine> MultiPVTool::allocate_region( WorldCanvas *wc, double x1, double y1, double x2, double y2, VOID * ) const {
		...
		// return std::shared_ptr<viewer::PVLine>(result);
	...
	bool MultiPVTool::create( viewer::region::RegionTypes /*region_type*/, WorldCanvas *wc,
		...
		std::shared_ptr<viewer::PVLine> result = allocate_region( wc, pts[0].first, pts[0].second, pts[1].first, pts[1].second, region_specific_state );
