...
#define CHECK_MOUSE_STATE(STATE)											\
	if ( state & viewer::region::STATE ) {									\
		...
		if ( ct_it != count_map.end( ) )									\
			...
			count_map.insert( state_count_type::value_type( viewer::region::STATE,	\
				std::shared_ptr<viewer::region::region_list_type>(new viewer::region::region_list_type( )) ) ); \
			ct_it = count_map.find(viewer::region::STATE);					\
			if ( ct_it != count_map.end( ) )								\
				(*ct_it).second->insert(region);							\
		}																	\
	...
	std::shared_ptr<viewer::region::region_list_type> RegionTool::State::regions( viewer::region::PointLocation loc,
	        viewer::region::RegionSelect select ) {
		state_count_type::iterator ct_it = count_map.find(loc);
		if ( ct_it != count_map.end( ) ) {
			return select == viewer::region::SelectAny ? (*ct_it).second : filter((*ct_it).second,select );
		} else {
			return std::shared_ptr<viewer::region::region_list_type>(new viewer::region::region_list_type( ));
		}
	}
	...
	std::shared_ptr<viewer::region::region_list_type> RegionTool::State::filter( std::shared_ptr<viewer::region::region_list_type> orig,
	        viewer::region::RegionSelect select ) {
		std::shared_ptr<viewer::region::region_list_type> result(new viewer::region::region_list_type( ));
