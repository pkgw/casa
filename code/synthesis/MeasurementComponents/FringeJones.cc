...
public:
    ...
    DelayRateFFT(SDBList& sdbs, Int refant) :
        ...
        // This is all now kind of dangerous, I guess?
        ...
        for (Int ibuf=0; ibuf != sdbs.nSDB(); ibuf++) {
            ...
            for (Int irow=0; irow!=s.nRows(); irow++) {
                ...
                // cerr << "f_index " << f_index << " t_index " << t_index << " spwchans " << spwchans << endl;
...
least_squares_driver(SDBList& sdbs, Matrix<Float>& param, Int refant, const std::set<Int>& activeAntennas) {
    ...
    for (size_t icor=0; icor != bundle.get_num_corrs(); icor++ ) {
        ...
        cerr << "Setting parameter vector for solver!" << endl;
        ...
        for (size_t iant=0; iant!=bundle.get_max_antenna_index()+1; iant++) {
            cerr << "For ant " << iant << " of " << bundle.get_max_antenna_index() << endl;
            if ( !bundle.isActive(iant) ) {
                cerr << "skipping " << endl;
                continue;
            }
            ...
            cerr << "For ant " << iant << ", psi0 = "    << psi0 << " psi1 = " << psi1 << " diff= " << psi1 - psi0  << endl;
        ...
        cerr << "Correlation " << icor << endl
             << "Summary from method '" << gsl_multilarge_nlinear_name(w) << "'" << endl
             << "Sub method " << gsl_multilarge_nlinear_trs_name(w) << endl
             << "number of iterations: " <<  gsl_multilarge_nlinear_niter(w) << endl
             << "function evaluations: " << f.nevalf << endl
             << "Jacobian evaluations: " << f.nevaldf2 << endl
