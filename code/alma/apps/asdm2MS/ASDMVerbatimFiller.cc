...
//ASDMVerbatimFiller::ASDMVerbatimFiller(MS* ms_p, const std::set<const ASDM_TABLE_BASE*>& table) {
...
void ASDMVerbatimFiller::fill(const asdm::ASDM& asdm) {
    for (std::set<ASDM_TABLE_BASE*>::const_iterator iter = table_.begin(); iter!=table_.end(); ++iter)
    (*iter)->fill(asdm);
}
