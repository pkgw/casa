...
public:
	 ...
	 * @param eoss the EndianOSStream to be written to
	 */
	static void toBin(const std::vector<std::vector<vector<Speed> > >& angle,  EndianOSStream& eoss);
	/**
	 ...
	 */	 
	 static std::vector<std::vector<vector<Speed> > > from3DBin(EndianIStream & eis);	 
