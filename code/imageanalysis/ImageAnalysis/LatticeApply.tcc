...
#ifndef CASA_LATTICES_LATTICEAPPLY_TCC
#define CASA_LATTICES_LATTICEAPPLY_TCC
...
namespace casa { //# NAMESPACE CASACORE - BEGIN
...
template <class T, class U>
void LatticeApply<T,U>::lineMultiApplyOld (PtrBlock<MaskedLattice<U>*>& latticeOut,
                      const MaskedLattice<T>& latticeIn,
                      LineCollapser<T,U>& collapser,
                      uInt collapseAxis,
                      LatticeProgress* tellProgress)
				      ...
				      LatticeProgress* tellProgress)
{
    ...
    const IPosition& inShape = latticeIn.shape();
    /*
    IPosition ioMap = prepare (inShape, shape,
			       IPosition(1,collapseAxis), -1);
			       */
    IPosition outPos(outDim, 0);
    IPosition outShape(outDim, 1);
    ...
    */
    ...
    // Inefficient, and then do the accounting for the input
    // lines in memory
    ...
    for (latIter.reset(); ! latIter.atEnd(); ++latIter) {
        ...
        // nSets = chunkShape.keepAxes(displayAxes).product();
        chunkSliceStart = 0;
        ...
        for (uInt k=0; k<nOut; ++k) {
            ...
            if (latticeOut[k]->hasPixelMask()) {
                ...
                //latticeOut[k]->putMaskSlice(resultArrayMask[k], cp);
                }
            }
        }
        if (tellProgress != 0) {
